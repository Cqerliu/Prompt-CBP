# This file is used to encapsulate data and convert DNA sequence data into computer recognizable numbers (tokens)

import torch
import torch.utils.data as Data

# Convert sequences to tokens and pad the sequence length
def input_token(train_sentences, train_labels, test_sentences, test_labels, tokenizer):
...
def input_token_test(test_sentences, test_labels, tokenizer):
    test_inputs = [tokenizer(sentence, return_tensors='pt')["input_ids"].squeeze() for sentence in test_sentences]
    test_inputs = torch.nn.utils.rnn.pad_sequence(test_inputs, batch_first=True, padding_value=tokenizer.pad_token_id)
